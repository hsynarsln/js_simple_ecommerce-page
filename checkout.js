...
window.onload = () => {
  window.localStorage.setItem('taxRate', taxRate); //kalıcı
  localStorage.setItem('shippingPrice', shippingPrice);

  window.sessionStorage.setItem('taxRate', taxRate); //browser'ı kapatıp açtığında kaybolur
...
[...quantityControllerDivs].forEach(quantityControllerDiv => {
  ...
  quantityControllerDiv.firstElementChild.addEventListener('click', () => {
    ...
    // }
    quantityP.innerText = parseInt(quantityP.innerText) - 1; //önce 0'a düşürdük
    if (quantityP.innerText == '0') {
      //0'a düştüğünde ürünü kaldırıyoruz
      alert('product will be removed!');
      quantityControllerDiv.parentElement.parentElement.remove(); //parentin parentine ulaşarak yapıyoruz
    }
    calculateProductTotal(quantityP); // Aşağıdaki fonksiyonun ierisine aktaracağımız değişken olan nesne
  });
  //plus button
...
const calculateCartTotal = () => {
  ...
  let shipping = subtotal > 0 ? shippingPrice : 0; //subtotal 0'dan büyükse shippingPrice' al değilse 0 al.
